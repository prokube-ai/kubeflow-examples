from kfp import dsl
from kfp.dsl import Input, Output, Dataset, Markdown, Model
from kfp import compiler

REGISTRY = ""  # Replace registry!
COMPONENTS_IMAGE = f'{REGISTRY}/kubeflow-examples/chem-util'
...
@dsl.container_component
def evaluate(input_data: Input[Dataset], model: Input[Model], viz: Output[Markdown]):
    return dsl.ContainerSpec(
        image=COMPONENTS_IMAGE,
        command=["/opt/conda/bin/python", "chem-util.py"],
        args=["evaluate", "-i", input_data.path, "-m", model.path, "-o", viz.path])
...
def chem_classification_pipeline(n_bits: int, n_trees: int):
    ...
    evaluate(input_data=split_data.outputs['output_test'], model=train_model.output)
